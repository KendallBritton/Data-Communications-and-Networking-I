...
int serverFd = socket(AF_INET, SOCK_STREAM, 0); //Checks socket file descriptor
if (serverFd < 0){
...
sockaddr_in newSockAddress; //Receive a request by accepting it
socklen_t newSockAddressSize = sizeof(newSockAddress); //Needs new address to connect

//Accepts and creates a new socket file descriptor
int newFd = accept(serverFd, (sockaddr *) &newSockAddress, &newSockAddressSize);
if (newFd < 0){
    cerr << "Error accepting the request" << endl;
    return -1;
}
...
while(1){
    ...
    bytesRead += recv(newFd, (char*) &message, sizeof(message), 0);
    ...
    if (data == "exit"){
        ...
        send(newFd, (char*) &message, strlen(message), 0);
        break;
    }

    //Send the message to client
    bytesWritten += send(newFd, (char*) &message, strlen(message),0);
}

close(newFd); //Closing the socket descriptors
close(serverFd);
